...
This is ``sheaf.annual`` (``sheaf/annual/core.py``), one tax per year. The crisis
market is the Gate 0 24-step spine; the crisis game is types slow / actions τ_t
on that spine (Headey 2011). See diagnostics/GAME_CLOCK.md. Crisis scores:
``scripts/score_subannual_crop.py``. This script is not a reason to skip them.
...
from sheaf.annual import build_countries, SheafModel, GRAINS
