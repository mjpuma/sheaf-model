...
imprecisely and are worth pinning down. First, the unmet-demand term is
one-sided,
\begin{equation}
...
than an edge case. Second, \(p^{\mathrm{tr}}_t\) is weighted by this
period's shipments and uses this period's offer prices \(q_{i,t}\) from
\eqref{eq:ask}, not the lagged \(q_{i,t-1}\) that entered the allocation in
\eqref{eq:reweight}. Offer prices are therefore updated twice within one
