...
START_YEAR = 2006
END_YEAR = 2008
AUTHOR_HIKE_2008 = 1.62
AUTHOR_MOY_MAXMIN = 1.45
AUTHOR_DRIFT_2006_11 = 1.004

# Knobs this object can copy from the Fig. 4 NetCDF global attributes.
CAN_SET = (
    "alpha_i=3.5",
    "alpha_nash=3.0",
    "zeta_penalty=1.0",
...
# Experiment pieces that are not in the repo / not on AgrimateParams.
CANNOT_SET = (
...
def fig4_experiment_region_path() -> None:
    """No EU28+Egypt / FAO FB path is vendored. Returns None on purpose."""
    return None
...
def choose_next_paste(summary: pd.DataFrame) -> tuple[str, str]:
    ...
    why = (
        ...
        "Item 3 / XI split is next (R3 if the price object is still undocumented)."
    )
...
def write_fig4_config_note(summary: pd.DataFrame, out_dir: Path) -> Path:
    ...
    lines = [
        ...
        "",
        f"Window: harvest+AMIS and undisturbed **{START_YEAR}–{END_YEAR}",
    ...
    lines += [
        "",
        f"Host regions: {avail['n_host_regions']} AgrimateRegionsWheat;",
        f"Egypt node={avail['host_has_egypt_node']}; EU-28="
        f"{avail['host_has_eu28']}. FAOSTAT FB files: "
        f"{avail['faostat_fb_files'] or 'none'}. USDA remains",
        f"`prepare_wheat` default={avail['usda_is_default']}.",
        f"`fig4_experiment_region_path()` is `{avail['region_path']}`.",
        "",
        "## harvest+AMIS 2006–08",
        "",
        "| arm | αI | ζ | N_for | hike 2008 | moy max/min | failed | unconv / n |",
        "|---|---:|---:|---:|---:|---:|---:|---:|",
        f"| default (`wheat_params`) | {ha_d['alpha_i']:g} | "
        f"{ha_d['zeta_penalty']:g} | {int(ha_d['n_for_months'])} | "
        f"×{_fmt(ha_d['hike_2008'])} | {_fmt(ha_d['moy_maxmin'], 1)}× | "
        f"{int(ha_d['failed'])} | {int(ha_d['unconverged'])}/{n_solves} |",
        f"| Fig. 4 knobs | {ha_c['alpha_i']:g} | "
        f"{ha_c['zeta_penalty']:g} | {int(ha_c['n_for_months'])} | "
        f"×{_fmt(ha_c['hike_2008'])} | {_fmt(ha_c['moy_maxmin'], 1)}× | "
        f"{int(ha_c['failed'])} | {int(ha_c['unconverged'])}/{int(ha_c['n_solves'])} |",
        f"| author Fig. 4d (this window) | 3.5 | 1 | 6 | "
        f"×{_fmt(ha_d['hike_2008_author'])} | "
        f"{_fmt(ha_d['moy_maxmin_author'], 2)}× | — | — |",
        f"| author Fig. 4d (published 2006–11) | 3.5 | 1 | 6 | "
        f"×{_fmt(AUTHOR_HIKE_2008)} | {_fmt(AUTHOR_MOY_MAXMIN, 2)}× | — | — |",
        "",
        ...
        "|---|---:|---:|---:|---:|",
        f"| default | {_fmt(un_d['last_first'], 3)} | "
        f"{_fmt(un_d['moy_maxmin'], 1)}× | {int(un_d['failed'])} | "
        f"{int(un_d['unconverged'])} |",
        f"| Fig. 4 knobs | {_fmt(un_c['last_first'], 3)} | "
        f"{_fmt(un_c['moy_maxmin'], 1)}× | {int(un_c['failed'])} | "
        f"{int(un_c['unconverged'])} |",
        f"| author (2006–08 on Fig. 4d) | "
        f"{_fmt(author_drift['last_over_first'], 3)} | "
        f"{_fmt(_moy_maxmin(_author_monthly('undisturbed', START_YEAR, END_YEAR)), 2)}× | — | — |",
        f"| author (2006–11 published) | {_fmt(AUTHOR_DRIFT_2006_11, 3)} | — | — | — |",
        "",
        "Host 2003–11 undisturbed last/first remains **1.63** (not re-run).",
        "",
        "## Did the knobs move Fig. 4 metrics?",
        "",
        f"Hike default ×{_fmt(moved['hike_default'])} → comparison "
        f"×{_fmt(moved['hike_comparison'])} vs author "
        f"×{_fmt(moved['hike_author'])} (this window).",
        f"Toward author hike: **{bool(moved['toward_hike'])}**.",
        f"moy max/min {_fmt(moved['moy_default'], 1)}× → "
        f"{_fmt(moved['moy_comparison'], 1)}× vs author "
        f"{_fmt(moved['moy_author'], 2)}×. Drop ≥2: "
        f"**{bool(moved['moy_drop_ge2'])}**.",
        f"Adaptive `moved`: **{bool(moved['moved'])}**.",
        "",
        ...
        "",
        f"Next paste from the adaptive table: **{next_id}**. {why}",
        ...
        "",
        "Protected (not written): "
        + ", ".join(f"`{n}`" for n in PROTECTED_THREE_SCENARIO) + ".",
        "",
    ]
    path = Path(out_dir) / "fig4_config.md"
    path.write_text("\n".join(lines) + "\n")
    return path


def write_dispatch(summary: pd.DataFrame, path: Path | None = None) -> Path:
    """Rewrite the living next-paste from this run's numbers."""
